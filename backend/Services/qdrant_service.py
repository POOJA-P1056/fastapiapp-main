...
def ensure_collection():
    collections = [c.name for c in qdrant.get_collections().
collections]
...
def embed_all_jobs(db: Session)->int:    
    ...
    def match_jobs_for_profile(profile: dict, top_k: int = 5) -> list[dict]:
        ensure_collection()
        profile_text = f"{profile.get('skills', '')} {profile.get('experience', '')} {profile.get('education', '')}"
